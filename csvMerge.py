...
path = "/Users/sborda/Desktop/DigitalPreservation/PDFs/TikaOutput/"

# 2. creates list with files to merge based on name convention
file_list = [path + f for f in os.listdir(path) if f.endswith('output.txt')]
...
for file in sorted(file_list):
    csv_list.append(pd.read_csv(file, sep="\t", encoding='latin-1', dtype='unicode').assign(File_Name = os.path.basename(file)))
...
csv_merged.to_csv(path + 'tikaPDF_full.csv', index=False)
